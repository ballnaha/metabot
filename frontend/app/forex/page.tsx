...
const strategyLabel = (name: string) =>
  ({
    forex_intraday:       "Forex M15 Intraday",
    forex_trend_pullback: "Forex H1 Trend Pullback",
...
export default function ForexPage() {
  ...
  return (
    <Box sx={{ minHeight: "100vh", bgcolor: "#080d18" }}>
                ...
                <CardContent sx={{ p: 0, "&:last-child": { pb: 0 } }}>
                  ...
                  >
                    <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
                      <Stack direction="row" spacing={1} sx={{ alignItems: "center", mb: 2 }}>
                        ...
                        <Typography variant="overline" color="text.secondary" sx={{ fontWeight: 700, letterSpacing: 0 }}>
                          <Box component="span" sx={{ display: { xs: "none", md: "inline" } }}>ราคาคู่เงิน Forex Real-time</Box>
                          <Box component="span" sx={{ display: { xs: "inline", md: "none" } }}>ราคา Forex</Box>
                        </Typography>
                      </Stack>
                    ...
                    <Stack direction="row" spacing={0.75} sx={{ alignItems: "center", flex: { xs: "1 1 100%", md: "0 0 auto" }, display: { xs: priceTableOpen ? "flex" : "none", md: "flex" } }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 0.75, height: { xs: 34, md: 38 }, px: 1, flex: 1, minWidth: { xs: 0, md: 190 }, bgcolor: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 1, transition: "border-color 0.2s", "&:focus-within": { borderColor: "rgba(6,182,212,0.5)" } }}>
                        <Search size={13} color="#475569" />
                        <input value={priceSearch} onChange={(e) => { setPriceSearch(e.target.value); setSymbolPage(0); }} placeholder="ค้นหา EURUSD/GBPJPY..." style={{ background: "transparent", border: "none", outline: "none", color: "#e2e8f0", fontSize: "0.82rem", width: "100%", fontFamily: "inherit" }} />
                      ...
                      <IconButton size="small" onClick={() => runScan(true)} disabled={scanLoading || forexSymbols.length === 0} sx={{ width: { xs: 34, md: "auto" }, height: { xs: 34, md: 38 }, borderRadius: 1, px: { xs: 0, md: 1.5 }, bgcolor: "#06b6d4", color: "#fff", "&:hover": { bgcolor: "#0891b2" }, "&.Mui-disabled": { bgcolor: "rgba(6,182,212,0.3)", color: "rgba(255,255,255,0.4)" } }}>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <TableCell align="right" sx={{ py: 1.25 }}>
                                    <PriceDirection value={hasPrice ? fmt(tick!.bid, dec) : "—"} direction={tickDirections[sym]?.bid ?? "flat"} />
                                  </TableCell>
                                  <TableCell align="right" sx={{ py: 1.25 }}>
                                    <PriceDirection value={hasPrice ? fmt(tick!.ask, dec) : "—"} direction={tickDirections[sym]?.ask ?? "flat"} />
                                  </TableCell>
                                  <TableCell align="right" sx={{ py: 1.25, ...MONO, fontWeight: 650, color: "#cbd5e1" }}>{rowSpread !== null ? fmt(rowSpread, dec) : "—"}</TableCell>
                                  ...
                                  <TableCell align="right" sx={{ py: 1.25, pr: 2 }}>
                                    <Button
                                      ...
                                      sx={{ height: 32, borderRadius: 1, fontWeight: 700, fontSize: "0.82rem", textTransform: "none", borderColor: "rgba(6,182,212,0.3)", color: "#22d3ee", "&:hover": { borderColor: "#06b6d4", bgcolor: "rgba(6,182,212,0.08)" } }}
                                    >
      ...
      {/* ── Forex Backtest Dialog ────────────────────────────────── */}
      <Dialog open={btOpen} onClose={() => { if (!btLoading) setBtOpen(false); }} maxWidth="md" fullWidth
        slotProps={{ paper: { sx: { bgcolor: "#0d1321", border: "1px solid rgba(34,211,238,0.18)", backgroundImage: "none" } } }}>
        <DialogTitle sx={{ pb: 1 }}>
          <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
            <FlaskConical size={18} color="#22d3ee" />
            <Typography sx={{ fontWeight: 800, color: "#f1f5f9" }}>Forex Backtest</Typography>
          </Stack>
        </DialogTitle>
        <DialogContent dividers sx={{ borderColor: "rgba(255,255,255,0.07)" }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr 1fr", sm: "2fr 1fr 2fr 1fr" }, gap: 1.5, mb: 2 }}>
            <Box>
              ...
              <Select size="small" fullWidth value={btSymbol} onChange={(e) => setBtSymbol(e.target.value)}
                ...
                {forexSymbols.map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
              </Select>
            </Box>
            <Box>
              ...
              <Select size="small" fullWidth value={btTimeframe} onChange={(e) => setBtTimeframe(e.target.value)}
                sx={{ bgcolor: "rgba(255,255,255,0.02)", "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(255,255,255,0.1)" }, "& .MuiSelect-select": { color: "#fff" } }}>
                {["M15","M30","H1","H4"].map((tf) => <MenuItem key={tf} value={tf}>{tf}</MenuItem>)}
              </Select>
            </Box>
            <Box>
              ...
              <Select size="small" fullWidth value={btStrategy} onChange={(e) => setBtStrategy(e.target.value)}
                sx={{ bgcolor: "rgba(255,255,255,0.02)", "& .MuiOutlinedInput-notchedOutline": { borderColor: "rgba(255,255,255,0.1)" }, "& .MuiSelect-select": { color: "#fff" } }}>
                {strategiesForGroup(strategies, "forex").map((s) => <MenuItem key={s.name} value={s.name}>{strategyLabel(s.name)}</MenuItem>)}
              </Select>
            </Box>
            <Box>
          ...
          {btLoading && <Box sx={{ py: 6, textAlign: "center" }}><CircularProgress sx={{ color: "#22d3ee" }} /></Box>}
          {btResult && !btLoading && (() => {
            const r = btResult;
        ...
        <DialogActions sx={{ px: 3, py: 2, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <Button variant="contained" disabled={btLoading || !btSymbol} onClick={runBacktest}
            ...
            sx={{ bgcolor: "#06b6d4", "&:hover": { bgcolor: "#0891b2" }, fontWeight: 700 }}>
